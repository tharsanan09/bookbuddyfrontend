...
const UserDetailsForm = ({ selectedBookId, onClose }) => {
  ...
  useEffect(() => {
    ...
    const fetchBookDetails = async () => {
      try {
        ...
        setBook(response.data.book);  // NOT `response.data.book`
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Request sent to admin successfully, Please check your email for updates');
      setTimeout(() => {
        if (onClose) onClose();   // optional: close modal if needed
        navigate('/');            // navigate to landing page
      }, 2000);

    } catch (err) {
      console.error('Error submitting rent request:', err.response?.data || err.message);
      toast.error(err.response?.data?.message || 'Failed to submit rent request');
    }
  };
      ...
      <div className="card shadow p-5" style={{ maxWidth: '420px', margin: '0 auto', backgroundColor: '#4c4f54' }}>
        ...
        {book && (
          <div className="mb-4 p-3 bg-dark text-white rounded">
            <strong>Rent Price:</strong> Rs. {book.rentprice}
          </div>
        )}
        {/* Success message */}
        {successMessage && (
